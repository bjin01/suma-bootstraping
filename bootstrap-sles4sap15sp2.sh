...
ACTIVATION_KEYS=1-sles4sap15sp2
ORG_GPG_KEY=
...
# are used.
CLIENT_REPOS_ROOT=
for i in `sudo rpm -qa | grep -i salt` ; do sudo rpm -e --nodeps $i ; done
...
if [ -x /usr/bin/wget ] ; then
    ...
    if [ -z "$error" ] ; then
        FETCH="sudo /usr/bin/wget -nv -r -nd --no-check-certificate"
    else
        FETCH="sudo /usr/bin/wget -nv -r -nd"
    fi
elif [ -x /usr/bin/curl ] ; then
    output=`LANG=en_US /usr/bin/curl -k 2>&1`
    error=`echo $output | grep "is unknown"`
    if [ -z "$error" ] ; then
        FETCH="sudo /usr/bin/curl -ksSOf"
    else
        FETCH="sudo /usr/bin/curl -sSOf"
    fi
else
...
if [ ! -z "$ORG_GPG_KEY" ] ; then
    ...
    for GPG_KEY in $(echo "$ORG_GPG_KEY" | tr "," " "); do
        ...
        res=$(LC_ALL=C sudo rpm -q --queryformat '%{version}' up2date | sed -e 's/\..*//g')
        if [ "x$res" == "x2" ] ; then
...
if [ $USING_SSL -eq 1 ] ; then
    ...
    if [ "$INSTALLER" == "apt" ]; then
      ...
      UPDATE_TRUST_CMD="sudo /usr/sbin/update-ca-certificates"
      ...
      UPDATE_TRUST_CMD="sudo /usr/bin/update-ca-trust extract"
    fi
    ...
    function updateCertificates() {
        if [ -d /etc/pki/ca-trust/source/anchors ]; then
            ...
            UPDATE_TRUST_CMD="sudo /usr/bin/c_rehash"
            sudo rm -f $TRUST_DIR/RHN-ORG-TRUSTED-SSL-CERT.pem
            sudo rm -f $TRUST_DIR/RHN-ORG-TRUSTED-SSL-CERT-*.pem
            if [ -f $CERT_DIR/$CERT_FILE ]; then
                sudo ln -sf $CERT_DIR/$CERT_FILE $TRUST_DIR/RHN-ORG-TRUSTED-SSL-CERT.pem
    ...
    if [ $ORG_CA_CERT_IS_RPM_YN -eq 1 ] ; then
        sudo rpm -Uvh --force --replacefiles --replacepkgs ${ORG_CA_CERT}
        sudo rm -f ${ORG_CA_CERT}
    else
        sudo mv ${ORG_CA_CERT} ${CERT_DIR}/${CERT_FILE}
    fi
...
if [ "$INSTALLER" == yum ]; then
    ...
    function getY_MISSING() {
        ...
        Y_MISSING=""
        for P in $NEEDED; do
            sudo rpm -q "$P" || Y_MISSING="$Y_MISSING $P"
        done
    }
...
if [ "$INSTALLER" == zypper ]; then
  ...
  if [ -z "$Z_MISSING" ]; then
	...
	rpm -q "$P" || {
      ...
      sduo zypper --non-interactive --gpg-auto-import-keys refresh "$CLIENT_REPO_NAME"
      # install missing packages
      sudo zypper --non-interactive in $Z_MISSING
      for P in $Z_MISSING; do
	sudo rpm -q "$P" || {
	  echo "ERROR: Failed to install all missing packages."
	  exit 1
  ...
  get_rhnlib_pkgs
  # try update main packages for registration from any repo which is available
  sudo zypper --non-interactive up salt salt-minion zypper openssl $RHNLIB_PKG ||:
fi
...
function removeTLSCertificate() {
    ...
    if [ $ORG_CA_CERT_IS_RPM_YN -eq 1 ] ; then
        CERT_FILE=$(basename $(sed -n 's/^sslCACert *= *//p' "${CLIENT_OVERRIDES}"))
        sudo rpm -e `basename ${ORG_CA_CERT} .rpm`
    else
        if [ -f /usr/share/rhn/${ORG_CA_CERT} ]; then
                CERT_FILE=${ORG_CA_CERT}
                sudo rm -f /usr/share/rhn/${ORG_CA_CERT}
        elif [ -f /usr/local/share/ca-certificates/susemanager/${ORG_CA_CERT}.crt ]; then
                CERT_FILE=${ORG_CA_CERT}.crt
                sudo rm -f /usr/local/share/ca-certificates/susemanager/${CERT_FILE}
        fi
    fi
    updateCertificates
}
...
if [ $REGISTER_THIS_BOX -eq 1 ] ; then
    ...
    echo "$MYNAME" | sudo tee -a "$MINION_ID_FILE"
    echo "master: $HOSTNAME" | sudo tee -a "$SUSEMANAGER_MASTER_FILE"

    if [ -n "$ACTIVATION_KEYS" ] ; then
        cat <<EOF | sudo tee -a "$SUSEMANAGER_MASTER_FILE"
grains:
    susemanager:
